...
function DirectoryManager() {
  ...
  const { data: residents, isLoading } = useQuery<any[]>({ queryKey: ["/api/residents"] });
  const { data: updatedInfo } = useQuery<{ updatedAt: string | null }>({ queryKey: ["/api/directory/updated"] });

  const uploadExcel = useMutation({
    mutationFn: async (file: File) => {
      const formData = new FormData();
      formData.append("file", file);
      const res = await fetch("/api/admin/directory/upload", { method: "POST", body: formData, credentials: "include" });
    ...
    onSuccess: (data) => {
      ...
      toast({ title: "Directory Updated", description: `${data.inserted} new residents added, ${data.skipped} existing updated.` });
    },
  ...
  const createResident = useMutation({
    mutationFn: async (data: { lotNumber: string; lastName: string; firstName?: string; phoneNumber?: string }) => {
      const res = await apiRequest("POST", "/api/residents", data); return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/residents"] }); setIsAddOpen(false); },
  });

  const updateResident = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const res = await apiRequest("PATCH", `/api/residents/${id}`, data); return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/residents"] }); setEditingResident(null); },
  });

  const deleteResident = useMutation({
    mutationFn: async (id: number) => { await apiRequest("DELETE", `/api/residents/${id}`); },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/residents"] }),
  });

  const deleteAllResidents = useMutation({
    mutationFn: async () => { await apiRequest("DELETE", `/api/residents`); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/residents"] }); toast({ title: "All residents deleted" }); },
    onError: () => toast({ title: "Error", description: "Failed to delete residents.", variant: "destructive" }),
  });

  const filteredResidents = residents?.filter((r) => {
    if (!searchTerm) return true;
    const term = searchTerm.toLowerCase();
    return r.lotNumber?.toLowerCase().includes(term) || r.lastName?.toLowerCase().includes(term) || r.firstName?.toLowerCase().includes(term);
  });

  const formatLastUpdated = (dateStr: string | null | undefined) => {
    if (!dateStr) return "Never";
    try { return format(new Date(dateStr), "MMM d, yyyy 'at' h:mm a"); } catch { return "Unknown"; }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Admin-only — residents cannot see this</p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Columns: Lot # · Name. Last updated: {formatLastUpdated(updatedInfo?.updatedAt)}</p>
              </div>
            </div>
            <input type="file" ref={fileInputRef} onChange={(e) => { const f = e.target.files?.[0]; if (f) { uploadExcel.mutate(f); e.target.value = ""; } }} accept=".xlsx,.xls" className="hidden" data-testid="input-excel-upload" />
            <Button variant="outline" className="gap-2" onClick={() => fileInputRef.current?.click()} disabled={uploadExcel.isPending} data-testid="button-upload-excel">
              {uploadExcel.isPending ? <><Loader2 className="w-4 h-4 animate-spin" /> Processing...</> : <><Upload className="w-4 h-4" /> Upload Excel</>}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div className="flex gap-2">
          <div className="relative">
            ...
            <Input placeholder="Search residents..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 w-64" data-testid="input-search-admin-residents" />
          </div>
          <Button variant="destructive" className="gap-2" disabled={deleteAllResidents.isPending || !residents?.length}
            onClick={() => { if (confirm(`Delete ALL ${residents?.length || 0} residents? This cannot be undone.`)) deleteAllResidents.mutate(); }}
            data-testid="button-delete-all-residents">
            {deleteAllResidents.isPending ? <><Loader2 className="w-4 h-4 animate-spin" /> Deleting...</> : <><Trash2 className="w-4 h-4" /> Delete All</>}
          </Button>
        </div>
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2" data-testid="button-add-resident"><Plus className="w-4 h-4" /> Add Resident</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Add New Resident</DialogTitle></DialogHeader>
            <ResidentForm onSubmit={(data) => createResident.mutate(data)} isLoading={createResident.isPending} />
          </DialogContent>
        </Dialog>
      </div>
      ...
      {isLoading ? (
                ...
                {filteredResidents.map((resident) => (
                  <tr key={resident.id} className="hover:bg-muted/30" data-testid={`row-resident-${resident.id}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Button variant="ghost" size="icon" onClick={() => setEditingResident(resident)} data-testid={`button-edit-resident-${resident.id}`}><Pencil className="w-4 h-4" /></Button>
                      <Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10"
                        onClick={() => { if (confirm(`Delete ${resident.lastName}?`)) deleteResident.mutate(resident.id); }}
                        data-testid={`button-delete-resident-${resident.id}`}><Trash2 className="w-4 h-4" /></Button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <Card className="p-12">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">Add residents or upload an Excel file to get started.</p>
          </div>
        </Card>
      )}
      ...
      <Dialog open={!!editingResident} onOpenChange={(open) => !open && setEditingResident(null)}>
        <DialogContent>
          ...
          {editingResident && <ResidentForm defaultValues={editingResident} onSubmit={(data) => updateResident.mutate({ id: editingResident.id, data })} isLoading={updateResident.isPending} />}
        </DialogContent>
      </Dialog>
    </div>
  );
...
function ResidentForm({ defaultValues, onSubmit, isLoading }: { defaultValues?: any; onSubmit: (data: any) => void; isLoading: boolean }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ lotNumber, lastName, firstName: firstName || undefined, phoneNumber: phoneNumber || undefined }); }} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-sm font-medium">Lot Number *</label>
          <Input value={lotNumber} onChange={(e) => setLotNumber(e.target.value)} required data-testid="input-lot-number" />
        </div>
        <div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="border-t pt-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Enter the resident's numeric user ID. The app links each device to that ID at login.
            </p>
            <div className="flex gap-2">
              <Input
